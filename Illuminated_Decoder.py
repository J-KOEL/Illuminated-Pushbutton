...
def load_data():
    ...
    # Manual LED lens conversion mapping
    led_lens_conversion_df = pd.DataFrame({
        "Label": ["RD", "GD", "YD", "WD", "LD", "AD"],
        "Code": ["C21", "C22", "C23", "C26", "C24", "C43"]
    })
    ...
    lens_conversion_lookup = {str(row['Label']).strip(): str(row['Code']).strip() for _, row in led_lens_conversion_df.iterrows()}

    return lightunit_lookup, lenscolor_lookup, voltage_lookup, lens_conversion_lookup

lightunit_lookup, lenscolor_lookup, voltage_lookup, lens_conversion_lookup = load_data()
...
if catalog_input:
    ...
    if normalized.startswith("10250T") and len(normalized) > 12:
        ...
        lens_conversion = lens_conversion_lookup.get(lens_code, "Unknown")

        lightunit_pn = f"10250T{lightunit_code}"
        lens_pn = f"10250T{lens_conversion}" if lens_conversion != "Unknown" else "Unknown"
        ...
        st.write(f"**Lens Color**: {lens_label} (Catalog suffix: `{lens_conversion}`)")
